...
import { estadoAcceso, mensajeSuscripcion } from '@/lib/entitlements';
 ...
 * @param {object} cliente - Fila del cliente (id, plan, suscripcion_vence, creado_en).
 ...
 */
export async function procesarIntent(cliente, resultado) {
  const clienteId = cliente?.id ?? cliente; // compatibilidad si llega solo el id
  ...
  // Intents que requieren acceso activo (plan de pago o prueba vigente).
  ...
  if (requiereAcceso.includes(resultado?.intent) && !acceso.activo) {
    return mensajeSuscripcion(acceso);
  }
  ...
  switch (resultado?.intent) {
    ...
    case 'saludo': {
      if (resultado.texto_confirmacion) return resultado.texto_confirmacion;
      const saludoBase =
        'Hola. Soy Fabbio, su asistente de recordatorios. Indíqueme qué necesita recordar y me encargo. ' +
        'Por ejemplo: "Recuérdame llamar al contador mañana a las 10".';
      // Si está en prueba, se lo recordamos amablemente.
      if (acceso.tipo === 'trial') {
        return (
          saludoBase +
          `\n\nEstá en su prueba gratuita (le quedan ${acceso.diasRestantes} día(s)).`
        );
      }
      if (!acceso.activo) {
        return saludoBase + '\n\n' + mensajeSuscripcion(acceso);
      }
      return saludoBase;
    }
